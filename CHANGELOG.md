...
- **Expanding a checklist card's completed-items dropdown in the notes grid let the page scroll far past its actual content, on both desktop and mobile.** Introduced in v1.4.6: whenever a checklist card's completed section opens, the grid reserves bottom padding so the expanding card (if near the bottom of a column) has room to grow before the masonry repack — triggered a few frames later via a `ResizeObserver` chain — catches up and repositions everything. Two prior attempts at this fix didn't hold up under testing: shrinking the reserved amount from a flat `50vh` to `maxCardHeightPx` only reduced the size of the standing dead zone without addressing why it was standing indefinitely (it was tied to "is a checklist still expanded," not to when the repack actually caught up); a follow-up that cleared the padding when the repack effect reached its internal "settled" state made things worse, because that effect's `useMemo` dependencies don't actually include the height-version counter, so a pure card-height change frequently never re-ran it — collapsing a card could leave the grid stuck oversized until a manual page refresh. The padding is now cleared by a plain, self-contained `setTimeout` (400ms) started every time a checklist toggles, independent of whether any other effect happens to re-run — a guaranteed, low-risk clear that doesn't depend on the masonry repack's internal memoization behavior.
