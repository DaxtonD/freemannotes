...
- **Expanding a checklist card's completed-items dropdown in the notes grid let the page scroll far past its actual content.** A prior fix reserved a flat `padding-bottom: 50vh` on the grid section whenever any checklist card had its completed section expanded, intended to keep other cards from visually shifting when the expanding card grows. 50vh is unrelated to how tall a card can actually get and doesn't scale with the user's own card-height preference, so it consistently over-reserved — on most viewports letting the grid scroll roughly half a screen past the last card. The reservation now uses the user's own `maxCardHeightPx` Appearance setting (the same ceiling `NoteCard` itself uses for a checklist card's expanded height) instead of a viewport-relative guess, kept live via a ref so a mid-session Appearance change is picked up immediately.
