...
### Fixed
- **Deleting a shared note from the editor put it in the OWNER's trash**, not yours. `trashNote()` writes straight into the note's own Yjs doc, and for a shared alias that doc belongs to whoever shared it with you — so an editor-role collaborator hitting delete quietly trashed the note for everyone, while still showing up as a collaborator on it. The grid's card menu already knew to leave a shared note instead of trashing it; the editor's delete button just never got the memo. Both now go through the same "leave" path, and the menu entry is honest about it: it reads **Leave note**, matches the wording in the Collaborators panel, and asks you to confirm first (you can't get back in without a new invite).
- **Accepting a share from the inbox could flash "You no longer have access to this note"** for a note you very much still had access to. The placements list a note's access is checked against starts empty on every load and catches up a beat later; a stale refresh-failure could also wipe it outright instead of just leaving it stale. Fixed both: a failed refresh no longer nukes what we already knew, and the "you lost access" screen now has to see that for real, continuously, for a second and a half before it says so — long enough to survive a slow connection, short enough that an actual revoke still shows up promptly.
- **Cross-workspace @mentions could switch you into a workspace you don't belong to** and then watch the app decide that workspace "doesn't exist" a moment later (because, correctly, you're not in it) — full-screen "workspace deleted" notice, inbox emptied, reload required. The access-check endpoint now says *why* you have access (real membership vs. a share), and the client only switches workspace for the former.
- **The checklist autocomplete could close your keyboard and scroll the list out from under you.** Accepting a suggestion that matched an already-completed item un-completed that row and deleted the one you were typing in — which unmounted your editor, which dropped focus, which closed the keyboard, which changed the layout, which is where the scroll jump came from. It now hands focus to the revived row in the same gesture instead.
- **Clearing your browser's site data lost your remembered workspace.** The device id that workspace preferences are keyed by lives in localStorage; wipe it and you get a brand-new id with no history, which used to fall back to "whichever workspace sorts first" instead of the one you were actually using. Login and the workspace list now also check which workspace you were last active in on ANY of your devices before giving up and guessing.
- **Attachment browser modals (images, links, documents, drawings) let the note grid scroll behind them.** `useBodyScrollLock` only ever stopped scrolling reaching all the way to `<body>`; a touch gesture that starts inside the modal's own list — especially in list view, or a view shorter than the modal — was never touching body in the first place, so it kept going. Every scrollable surface in these modals now stops the buck itself.
- **The same modals had an uncovered strip between the modal's header and the panel's own section header**, letting note cards peek through underneath, and the section label ("Links", "Documents", "Images") was printed twice — once by the modal, once by the panel. Both fixed; the panel now knows when it's rendering inside the browser modal and skips its own redundant label.
- **On touch, the PDF markup select tool grabbed an element the instant you touched it**, which meant trying to scroll past a stamp or symbol dragged it around instead. A touch on something not already selected now selects it and lets the page scroll like normal; dragging it needs a second touch, same as reaching for its resize handles already required. Comment pins are the exception — those still open on the first tap.
- Opening the PDF markup tools for the first time landed on the pen. It's the select tool now.

### Changed
- **Note banners are per-user again**, like note colors already were. Picking a banner used to write into the note's shared metadata, so changing it on a note you collaborate on changed it for everyone. A note's existing shared banner is still what everyone sees until you personally pick something else — nothing disappears, it just stops being everyone's problem when one person redecorates.
- **Symbols placed in PDF markup got a rotate handle.** Select one and a handle appears above it — drag it around like a clock hand and it snaps to the same quarter turns the toolbar's rotate button already does, just without having to go find the button.
...
### Fixed
- **Cards no longer shift and oscillate while scrolling.** The grid kept a map of every card's real measured height, and the *packer* used it — but the **virtualizer**, which is what actually decides each column's total height and every card's scroll offset, was handed a generic estimate instead, even for cards whose true height we already knew. So it laid the columns out from guesses, and then every card that scrolled into view reported its real size and shoved everything below it. One lookup consulted the measurements, the other didn't, and they'd been sitting eight lines apart this whole time. Worth being honest about how we got here: 1.12.0 fixed how a card *computes* its height and an earlier attempt fixed what a card *starts* from, and neither made the slightest difference, because neither touched what the virtualizer was actually told.
- **One note could make the entire grid lurch.** A card preview rendered **every block in the note** and then let CSS clip it — so a note with a few thousand lines built a few thousand DOM nodes for a preview that shows about twenty, and because virtualization remounts cards as you scroll, it paid that cost again every single time the card drifted back into view. Capped at 120 blocks, which still overfills the tallest possible card about twice over. This also quietly explains a "why is dev slow but production fine" mystery we'd blamed on the GPU, then antivirus, then the dev build: it was one note, and it only existed in the dev database.
- **The checklist progress orb fired its whole pulse animation just from opening a note.** The bar's *travel* was already suppressed while a note settles in — but the orb's flash and its two staggered ring pulses are CSS keyframes, and CSS animations run whenever an element mounts. Opening a checklist note mounted the orb and set all three off. Now they stay off until an actual check or uncheck.
- **Preferences → Appearance flashed your previous pane before snapping back to Theme.** The modal renders nothing while closed rather than unmounting, so it remembers the last pane you were on; reopening painted that stale pane and only *then* ran the reset. Moved the reset to before the browser paints. Same bug's evil twin also fixed: changing the theme while sitting on the Language or Display pane used to drag you back to Theme mid-edit.

### Changed
- **Column balancing rebuilt as two layers.** The canonical layer deals notes round-robin, so the same notes appear in the same reading order whether you're on a two-column phone or a six-column desktop. The display layer then rebalances *only the bottom of the grid*, migrating the lowest note of the tallest column to the shortest. Cards above that never move — the note you know is "left column, partway down" stays exactly there. The previous balancer could only shift the boundary *between* columns, which re-deals the entire tail and, with tall cards, was too blunt to even them out at all, so it gave up and left the gaps you could see.
- **Dragging a card in grid view now swaps it with the card at the destination**, rather than inserting it and pushing everything along. That's not a stylistic preference: a note's column is determined by its position in the canonical order, so an insertion shifts every position in between and sends all of those notes hopping between columns — the exact "it shuffles a whole bunch of unrelated notes" behaviour we'd specifically set out to avoid. Trading two positions moves precisely two notes. Dropping past the end of a shorter column, or onto the other side of the pinned/unpinned divide, still inserts — swapping across that divide would silently change whether a note is pinned.
...
### Added
- **A "Move to trash" option for notes someone shared with you.** Previously the only way to get rid of a note shared to you was to open it, dig into the Collaborators modal, and hit "Leave Note" — a completely undiscoverable path unless you already knew "leaving" was the mechanism. It's now a normal menu entry, right where you'd look for it, and does the exact same thing under the hood (same real-time sync to everyone else) — you just don't need to know that. Works for view-only shares too, which is more than the old modal-only path could say: that one was accidentally locked to editors only.
- **"Remove completed items"** in the checklist menu — one click empties out the completed section instead of hand-deleting a hundred checked-off grocery items one at a time.
- **Checking off (or autocompleting into) an item that matches an already-completed one now reactivates the old one instead of making a duplicate.** Buy apples every week? You get one "apples," not fourteen stacked in the completed section forever. Reactivates whichever completed match was checked off most recently if there's more than one, so it doesn't have to guess; anything the completed item was carrying — sub-items, a count, formatting — survives, since we're waking the original back up, not deleting it and typing a fresh one.

### Fixed
- **Closes out 1.9.1's "3-dot menu overshot" known issue.** The corner menu icon on checklist cards without a preview was never going to line up right as long as it was a reserved empty band below the row it's supposed to share a centerline with — centering a glyph in dead space below a row is not the same thing as centering it on that row, no matter how many times you nudge the number. Rebuilt it as an actual flex sibling of the toggle row so it shares the row's real height by construction, then went back and fixed the *other* menu icon (text/drawing cards) to line up with it by matching the same right-edge inset instead of eyeballing a center point — turns out matching an edge holds regardless of glyph width; matching a "center" never quite does.
- **Closes out 1.9.1's "cards render taller than they should, then shrink" known issue, at least for checklists and drawings** (the two card types that actually had this). Checklist cards used to guess their own header/rail height using a fake placeholder card width because the real one wasn't known yet at render time — now it is, threaded down from the grid's own column math, so the first paint is correct instead of correct-a-frame-later. Native drawing card previews had the same disease for a different reason: the skeleton and the real thumbnail were sized by two genuinely different mechanisms (a fixed minimum vs. the image's real aspect ratio), which is exactly why they never quite matched up — each drawing's real aspect ratio is now captured the moment its thumbnail is generated and reserved up front, so there's nothing left to correct.
- **Checklist cards with a URL preview could still, on rare occasion, cascade through several visible resizes right after a cold page load** — item count would shift (6 active + "3 more" would become 7 + "2 more"), the preview would get shoved around, the corner menu icon would end up somewhere else entirely. Root cause: the item-count budget was reading the *live* rendered height of the link preview, and that preview's real content (title, description) can keep arriving for several seconds after the note first loads while it hydrates over the network. Every time real data landed, the budget recalculated, and recalculating the budget can change how many items fit. It now budgets off a fixed worst-case number derived from the preview's own CSS clamp instead of whatever the network happened to deliver by that render — the number literally cannot be exceeded by real content, so there's nothing left to react to mid-hydration.
- **Collapsed checklist cards could clip the completed-items row behind the corner menu's hover-reveal dock**, at certain combinations of text size and card-height preference. Took real on-device DevTools measurements (thank you for those) to actually nail this one instead of guessing at another number — turned out the card's own "never clip, grow past the height cap if you truly have to" safety net only ever protected the empty-chrome case, not the case where the real, already-decided-on item count needed a few more pixels than the estimate assumed it would. It clipped instead of growing, silently, right where the dock est real estate lives. Now honors the actual content it committed to rendering, not just the bare minimum.
- **A shared note could show up in your own Trash if the *owner* trashed it — and you could then hit Restore on it**, un-trashing someone else's note without them doing anything. Trash state lives on the same shared document both of you are looking at, so this was always going to sync across; the fix isn't in the sync, it's that we found the one action here that was never actually yours to take and turned it off. Leaving still works, restoring someone else's note doesn't.
- **The checklist progress bar animated its "N of X completed" fill every single time a note was opened**, not just when something was actually checked or unchecked. It has no way to tell "this note's data is still settling in from local storage" apart from "a checkbox just got clicked" — both look identical from where the bar is standing, a completed-count change between renders. Now anything that happens in the first moment after a note opens gets treated as settling in, not a real toggle, and just snaps into place instead of playing the whole animation like you just checked off a dozen items you didn't touch.
- **A URL preview on a note colored "white" (or several other light colors) rendered as a barely-legible grey chip with pale grey text.** The link-preview component never got wired to the per-note color system at all — it was hardcoding this app's own dark-theme surface/text colors regardless of what color the actual card was, which happened to look fine by accident on darker note colors and fell apart completely on light ones. Now follows the note's own resolved color scheme, the same one the rest of the card already uses, and falls back to the exact same colors as before everywhere else this component renders.
- **The completion pulse glow got a hard, ugly clip on whichever edge was closest to the scrollable list's own boundary** — worst on the very first item, whose glow had nowhere to bloom upward into at all. This is not a z-index bug and bumping one wouldn't have fixed it (the glow already paints above the row, always did) — it's that a scrollable container can't opt into vertical scrolling without also clipping horizontal overflow, a CSS rule nobody remembers agreeing to. Added a little breathing room around the list instead of touching the glow itself, which someone clearly spent real time getting to look right and didn't need us re-tuning it to fit a smaller box.

### Known Issues
- Drawing thumbnails and checklist chrome height are now deterministic; text-note and plain image-attachment cards weren't part of this audit and may still show a similar, smaller settle on cold start. Not reported as a live problem yet — flagging so it doesn't look like an oversight if it turns up.
- Workspace pre-warming (making a workspace switch you've never visited this session behave identically to one you have) was investigated and deliberately not built — the existing background preload already appears to close most of the practical gap, and actually solving the rest would mean touching connection-lifecycle internals for a benefit nobody's confirmed is still missing in practice.
