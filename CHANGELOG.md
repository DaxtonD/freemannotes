...
- **Banner picker did not actually show banners offline.** `listNoteBanners` waited on a network fetch even when a cached banner list already existed in localStorage. It now returns the cached list immediately (stale-while-revalidate) and refreshes in the background, and the cache is warmed once after login so the picker has data on its very first offline use.
- **Clicking a link/mention to a trashed or deleted note silently opened it (or crashed).** `/api/notes/:id/access-check` now reports `trashed` status by reading the note's live Yjs metadata (in-memory doc if loaded, otherwise the persisted state), the same pattern already used by the trash-empty endpoint and the reminder trash guard. Clicking a mention chip pointing at a trashed note now shows a toast with a "Restore note" action instead of opening it; a permanently-deleted note shows "This note no longer exists." instead of opening stale content. Both the rich-text editor's mention chips and the checklist-item preview chips funnel through the same gated `openLinkedNote` handler in `App.tsx`, so the fix applies uniformly. A browser-history entry is now only pushed when navigation actually happens, fixing the corrupted forward/back stack that resulted from opening a note that should have been blocked.
- **Clicking an inbox @mention card for a trashed or deleted note silently opened it.** The inbox's `onOpenNote` handler never checked whether the note still existed — unlike the note-link chip click path, it went straight to opening the note. It now runs the same access-check gate: trashed shows a restore toast, and a permanently-deleted note shows "This note no longer exists." and auto-archives the now-useless inbox card so it doesn't keep coming back. Cross-workspace/shared-placement cards are unaffected by this change; a note that's still trashed but not yet emptied can still be restored from the toast.
- **Self-mentioning yourself in a brand-new note's first save produced no inbox notification.** The server intentionally skips mention-notification emission while a note's Yjs `metadata.pendingNew` flag is set (to avoid spamming notifications for drafts), and only flushes queued notifications when `/api/notes/:id/flush-mentions` is called — normally triggered by `closeNoteEditor`. The "Save" action on a brand-new note/checklist (`savePendingNewNoteAndClose`, and the equivalent for drawings, `saveDrawingEditor`) clears `pendingNew` but never called `flush-mentions`, so a self-mention added before the note's very first save was queued server-side but never flushed into a real `Activity` row — nothing else prompts the flush until the note's WebSocket room disconnects, which normal editor open/close never does. Saving a note for the first time now calls `flush-mentions` immediately after finalizing, matching what already happens when closing an already-saved note.
- **Swiping an inbox card to archive it also dragged the mobile sidebar open.** `App.tsx` has a `document`-level `touchmove` listener (bubble phase) that opens the mobile sidebar on a left-edge right-swipe (touches starting within 36px of the edge). `InboxView`'s own swipe-to-dismiss handler didn't reserve that zone, so a card swipe starting near the screen edge was claimed by both gestures at once. The card's swipe now leaves any touch starting within that same 36px edge zone completely alone (no `preventDefault`/`stopPropagation`) so it passes straight through to the sidebar gesture; past that zone, the card's `touchmove` handler calls `stopPropagation()` so its own swipe never bleeds through to the sidebar listener either. The two gestures now split the screen width instead of racing for the same touch.
- **Clicking an inbox card for a mention in a different workspace crashed the app or opened an empty note.** Inbox cards are cross-workspace — a mention in any workspace you belong to shows up regardless of which workspace is currently active. When the target note had no existing share-placement alias, `onOpenNote` fell through to opening it by bare `noteId` under whichever workspace was currently active, resolving to the wrong Yjs room (`${activeWorkspaceId}:${noteId}` instead of `${notesActualWorkspaceId}:${noteId}`) — at best an empty stub doc, at worst a server-side "forbidden namespace" WS rejection (rooms are authorized per activated workspace) that crashed the app. Opening a direct cross-workspace mention (no placement) now confirms access first, then switches the active workspace to the note's owner (the same `activateWorkspaceFromSidebar` used by the workspace switcher) before opening it — trashed/missing are still caught by the same access-check gate as every other note-link entry point.
- **Checklist drag ghost showed a stale @mention avatar and a shifted checkbox.** `@hello-pangea/dnd`'s `renderClone` fully unmounts the real checklist row (including the live `ReferenceChip`/`useLiveUserAvatar` node view) and renders a hand-built preview through a plain, non-reactive `renderRichPreview` helper instead — one that painted mention avatars from `node.attrs.avatarUrl`, a value captured once at mention-insertion time and never updated after a profile picture change. `renderRichPreview` now accepts the same `liveAvatarLookup` (from `useLiveAvatarUrlLookup`) that `NoteCard`'s static preview pipeline already uses, so the drag clone and non-active-row previews both resolve the current avatar instead of the stale one. Separately, the clone's checkbox was a bare `<input>` while the live row wraps it in a `.checklistCheckboxHitArea` label that the CSS positions differently — the clone's checkbox now uses the same wrapper, so it no longer visibly shifts position for the duration of the drag.
- **New/draft checklists silently dropped @mention and note-link chips from row previews.** `ChecklistEditor.tsx`'s own `renderRichPreview` (used before a checklist's first save) had no handling for `reference` nodes at all — a mention or note link in a collapsed/inactive row simply vanished from the preview text instead of showing a chip, only reappearing once the row was reactivated. It now renders the same reference chip (icon/avatar + label) as `NoteEditor.tsx`'s equivalent, including the live-avatar-lookup fix above; note-type chips render as static (non-clickable) since the draft flow has no note-navigation callback to wire up.
- **Expanding a checklist card's completed-items dropdown in the notes grid let the page scroll far past its actual content, on both desktop and mobile.** Introduced in v1.4.6: whenever a checklist card's completed section opens, the grid reserves bottom padding so the expanding card (if near the bottom of a column) has room to grow before the masonry repack — triggered a few frames later via a `ResizeObserver` chain — catches up and repositions everything. Four attempts landed here: shrinking the reserved amount from a flat `50vh` to `maxCardHeightPx` only reduced the size of the standing dead zone, not why it stood indefinitely; clearing it when the repack effect reached its internal "settled" state made things worse, since that effect's `useMemo` deps don't include the height-version counter and frequently never re-ran from a pure card-height change, leaving the grid stuck oversized after a collapse until a manual refresh; a `setTimeout`-based clear (400ms, independent of whether any other effect re-runs) fixed the standing dead zone but introduced a smaller issue — collapsing while scrolled to the bottom let cards shift up correctly, then the padding's removal 400ms later abruptly shrank the scrollable area and snapped the scroll position (and every visible card) down the screen; animating that removal with a 220ms `padding-bottom` transition smoothed the snap, but the buffer is real scrollable space for the full 400ms it's reserved — scrolling into it immediately after a toggle (before the timeout fires) still produced a visible slide-back once it cleared, repeatable on every consecutive toggle. The buffer is now also paired with a temporary `window` scroll listener (installed the instant the buffer is applied, removed the instant it's cleared) that caps `scrollY` to the section's real, buffer-excluded bounds for as long as the buffer is reserved — so there's no dead zone left to scroll into in the first place, whether the user scrolls immediately or waits. That listener's first version introduced its own bug: correcting `scrollY` via `scrollTo()` dispatches another `scroll` event, which re-entered the same handler and re-corrected against its own prior correction — with any sub-pixel rounding difference between the computed target and what the browser actually applied, that self-triggering loop produced severe, rapid up/down oscillation on the very first scroll attempt after expanding a dropdown. The handler now batches to at most one correction check per animation frame and only corrects when more than 1px over the bound, which breaks the self-triggering loop instead of reacting to it after the fact. That still wasn't enough on an installed iOS PWA: the clamp was hardcoded to `window`/`document.documentElement`, but `.test-harness-root.ios-standalone-pwa` is its own `overflow-y: auto` container there, so `window.scrollY` never moves and the clamp silently corrected nothing while the underlying oscillation-prone dead zone was still fully scrollable. The clamp now walks up from the grid section to find whichever ancestor is actually the scroll container (mirroring the existing `findScrollableAncestor` pattern already used in `NoteEditor.tsx`) and falls back to `window` only when none is found, so it targets the real scrolling element regardless of platform or display mode.
...
### Fixed
- **Held-backspace crash in note editor ("Maximum update depth exceeded").** Three `useSyncExternalStore` subscriptions in `NoteCard` each called `onStoreChange()` synchronously on every Yjs `afterTransaction` event. During held backspace, y-prosemirror fires ~30 transactions per second, pushing React past its 50-render depth limit and crashing the UI. All four Yjs-driven card preview subscriptions (`useTextNoteRichPreview`, `useChecklistItems`, `useDrawingThumbnail`, `extractedLinks`) are now RAF-throttled: rapid transaction bursts coalesce into at most one React re-render per animation frame. The `useChecklistItems` cache is still updated eagerly (synchronously) so the snapshot is always fresh even though the render is deferred.
- **Unpinning a note causes it to disappear and fly back to its old position.** Pin state is user-scoped and stored separately from the canonical Yjs `noteOrder`. When a note was unpinned, `pinnedFirst()` (applied at render time) removed it from the top, and the card animated back to wherever it sat in the pre-pin `noteOrder`. Fix: unpinning now writes the note's current visual position (index 0) into the canonical `noteOrder` via a Yjs transaction, treating the unpin as an implicit "keep it here" move. Works from both the card more-menu (`NoteGrid`) and the open editor toolbar (`App`).
- **Keyboard flash when navigating to a linked note on mobile.** Tapping a note chip (@reference) in a rich-text editor triggered `pointerdown` focus on the TipTap `contentEditable` before the click handler fired, causing the software keyboard to briefly appear then dismiss as the old editor unmounted. `handleReferenceClick` in `RichTextEditor` now blurs the active element synchronously at click time (before the async access-check fetch), so the keyboard starts dismissing immediately rather than after the navigation completes.
- **Notes grid visible through "Loading editor" overlay during note-to-note navigation.** The loading placeholder that renders while a new note's Yjs doc is being fetched used `var(--color-overlay)` (45% opacity) as its background, making the grid bleed through. Background changed to `var(--color-surface)` (opaque). Same fix applied to the `DrawingEditor` Suspense and error-boundary fallbacks.
- **Notes grid visible behind new editor on first render frame (mobile).** The `.fullscreenOverlay` wrapper uses `var(--color-overlay)` (45% opacity) as its background. On mobile, the editor card fills the full viewport during steady-state, but on the very first paint frame the card has not yet measured its height — leaving the semi-transparent overlay background exposed. The mobile (`pointer: coarse`) CSS override now sets an opaque `var(--color-surface)` background on `.fullscreenOverlay`, eliminating the single-frame grid bleed on editor open.
- **TypeScript errors in `NoteEditor.tsx` and `NoteCard.tsx`.** Six pre-existing type errors resolved: (1) `mediaSheetStyle` object cast to `React.CSSProperties` to allow CSS custom property keys; (2) `readonly ChecklistItem[]` snapshot cast to satisfy mutable undo stack slot; (3) `titleFieldRef` typed as `(HTMLTextAreaElement & HTMLInputElement) | null` — the ref is attached to both element types in different branches, and the intersection satisfies both JSX ref props while keeping all used methods (`scrollHeight`, `setSelectionRange`, etc.) type-safe; (4) `getChecklistItemRichPreviewJson` parameter widened to accept `Y.Map<any> | null` with an early-return null guard, preventing a potential runtime throw when a checklist item's map is absent; (5) `isElementLayoutMeasurable` return type changed to a type predicate (`element is HTMLElement`) so TypeScript narrows `card` to non-null after the guard; (6) `for...of` on `NodeListOf<HTMLElement>` replaced with `Array.from(textNodes)` for compatibility.
...
- **@ mention dropdown empty on cold start + offline.** `UserReferenceProvider` fell back to an empty list when the workspace members LS cache was absent (cleared or first session before the proactive fetch completed). The workspace list LS cache (`freemannotes.workspaceListCache.v1.<userId>`) — which already populates the sidebar offline — contains `ownerUserId`, `ownerName`, `ownerEmail`, and `ownerProfileImage` for every workspace the user belongs to. `initWorkspaceMembersCacheForUser` now uses this as a secondary fallback so workspace owners are available in the @ dropdown under the same offline conditions as the sidebar.
