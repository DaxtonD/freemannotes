...
- **Swiping an inbox card to archive it also dragged the mobile sidebar open.** `App.tsx` has a `document`-level `touchmove` listener (bubble phase) that opens the mobile sidebar on a left-edge right-swipe. `InboxView`'s own swipe-to-dismiss handler called `preventDefault()` but not `stopPropagation()`, so a card swipe starting near the screen edge — the same zone the sidebar gesture watches — kept bubbling past the card and drove the sidebar's drag-progress in sync with the card's swipe. The card's touchmove handler now also calls `stopPropagation()`, so a swipe that starts on a card is claimed by the card and never reaches the sidebar's edge-swipe listener.
