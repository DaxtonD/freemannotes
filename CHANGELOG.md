...
- **Expanding a checklist card's completed-items dropdown in the notes grid let the page scroll far past its actual content, on both desktop and mobile.** Introduced in v1.4.6: whenever a checklist card's completed section opens, the grid reserves bottom padding so the expanding card (if near the bottom of a column) has room to grow before the masonry repack — triggered a few frames later via a `ResizeObserver` chain — catches up and repositions everything. The padding was tied to "is any checklist card currently expanded" and only cleared once the user explicitly collapsed one, so the extra scroll room persisted for the *entire time* a dropdown stayed open, not just the brief window the repack actually needed it for — an initial fix that swapped the flat `50vh` reservation for a smaller `maxCardHeightPx`-based one only reduced the size of this standing dead zone, it didn't address why it was standing at all. The padding is now cleared unconditionally the moment the repack settles (it always ends with an accurate real height by then), regardless of whether a checklist is still expanded — so the reserved buffer only exists for the few frames it's actually doing anything, not for as long as the dropdown happens to stay open.
