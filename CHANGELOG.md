...
### Known Issue
- **Expanding a checklist card's completed-items dropdown in the notes grid reserves extra scroll room below the last card (unchanged since v1.4.6).** Several attempts to shrink/animate/clamp this reservation were tried and reverted after each introduced a worse regression, most recently a scroll-position clamp that oscillated severely on touch devices — the clamp's `scrollTo` correction fights the OS's own momentum/inertial scroll physics, which keeps animating independently of any JS correction. Reverted back to the original, known behavior (a flat reservation, cleared only when a completed section is explicitly collapsed) rather than ship a worse bug. Revisiting this will need a non-JS-reactive approach.
