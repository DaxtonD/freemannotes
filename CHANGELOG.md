...
- **Dragging an active checklist item near the bottom of the screen scrolled the completed-items section into view for no reason.** `@hello-pangea/dnd`'s auto-scroll only knows about the scroll container's edges, not that the completed section sits outside the active items' `Droppable` and can never be a valid drop target. A drag starting near that edge now clamps the container's `scrollTop` (reusing the same boundary the existing "scroll to reveal a new item" feature already computes) so the completed section stays off-screen for the duration of the drag — no wasted auto-scroll, and no visually dragging an item into a section it can never actually be dropped in. Fixed in both the saved-note editor and the new/draft checklist composer.
