...
- **Dragging an active checklist item near the completed-items section caused unwanted scrolling, including scroll oscillation when held at the boundary.** `@hello-pangea/dnd`'s auto-scroll is driven by its own internal pointer-tracking state, not the rendered drag clone — so an earlier attempt at this fix (reactively clamping `scrollTop` on `scroll` events) fought the library's own scroll loop frame-by-frame, producing visible up/down oscillation when the ghost was held at the edge, and still let a few completed rows peek into view since the reused boundary was tuned for a different feature. The real fix clamps the *input* coordinates before they ever reach the drag library: a new `clampChecklistDragY` (in `checklistDragState.ts`) caps the `y` position the custom pointer sensor (`dndSensors.ts`) reports to `@hello-pangea/dnd`, at the top edge of the completed-items section (registered per-drag via `registerChecklistDragMaxY`). Since the library derives both the dragged clone's rendered position *and* its auto-scroll trigger from this same reported position, capping it at the source freezes the ghost right at the boundary and stops auto-scroll from ever requesting a scroll past it — no reactive fighting, no oscillation. Fixed in both the saved-note editor and the new/draft checklist composer.
