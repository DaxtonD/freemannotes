...
**Server text extraction** (`server/noteDocumentPreview.js`): `pdf-parse` 2.x is a class (`new PDFParse({ data }).getText()`), not a function. Extraction failures must return their error, not an empty result — an empty result is how a broken extractor hid for a very long time. Uploads and the conversion queue call it through `extractDocumentTextInWorker` (`server/documentTextExtraction.js` → `documentTextWorker.js`): a worker thread, two at a time, 120 s, 1 GB heap, and it never throws. **Load `pdf-parse` on the main thread before any worker does:** if a worker is the first to load its native canvas library, the next worker crashes the whole process (an access violation on Windows). Both worker runners preload it; keep that if you add another. `sharp` is loaded lazily in `noteDocumentPreview.js` so the text worker never loads it.

**Office → PDF (optional Gotenberg):** `server/documentConverter.js` talks to Gotenberg's LibreOffice route; `server/documentConversionQueue.js` turns PENDING versions into a `view.pdf` next to the original, one at a time, so `uploadAccess.js` and version deletion already cover the copy. The state lives on the version (`conversionStatus`, `viewPdfPath`, `conversionError`). Outages and setup mistakes (unreachable, 401/403/404) never mark a file failed: the queue backs off from 30 s to 10 min and retries. A 400 fails the file at once; a 500, a timeout or output that isn't a PDF gets three tries. On startup, existing office files are queued when `GOTENBERG_URL` is set, and leftover PENDING rows are reset when it isn't, so devices never wait on a converter that isn't there. `/api/config` returns `documentConversion` so clients know which experience they're getting. To try it locally: `docker run --rm -p 3000:3000 gotenberg/gotenberg:8 gotenberg --chromium-disable-routes=true` and set `GOTENBERG_URL=http://localhost:3000`.

**First-page previews:** upload writes a generated text card (`preview.webp`/`thumb.webp`) straight away. `server/documentThumbnails.js` then renders page 1 of the PDF, or of the Gotenberg copy, into `preview-page.webp` (960 px) and `thumb-page.webp` (480 px) and repoints the version at them. Rendering runs in a worker thread (`documentThumbnailWorker.js`, pdf-parse's `getScreenshot` on `@napi-rs/canvas`) with a 20 s limit and a memory cap, because drawing a heavy scanned page on the main thread would stall every request and WebSocket. New names on purpose: the service worker caches images by URL, so overwriting the text card would keep it on screen forever on devices that had seen it. The queue picks work by query (PDF, or COMPLETE conversion, whose thumbnail isn't `thumb-page.webp` yet), runs at startup and after uploads and conversions, and skips a failed file until the next restart.

**PDF markup** lives in `src/components/NoteDocuments/markup/`. Coordinates are page units: pdf.js's viewport at scale 1, y down, page rotation applied, so markup stays on the same spot at any zoom or screen size. Stroke widths and font sizes are page units too. Everything read from storage or another device goes through `isMarkup` / `isPageScale` before it's drawn.

- One Yjs doc per document **version** (IndexedDB `freemannotes-markup:<versionId>`) with maps `markups`, `replies` (separate so two people replying at once both land), `scales` (page → `PageScale`) and `meta` (`lastCommentNumber`, deliberately not undoable). `Y.UndoManager` tracks the first three, local origin only.
- Symbol ids (`markupSymbols.tsx`) and stamp presets are stored in documents: never rename or remove one. An unknown symbol draws as a placeholder.
- Comment numbers come from `meta.lastCommentNumber` and are never reused. Duplicates from offline edits are renumbered by `planCommentRenumbering`, which is deterministic so every device agrees, and only runs for editors once synced.
- **Sync:** room `markup:<versionId>` over the normal `/yjs` socket. `server/markupRooms.js` authorizes it (version → note document → `resolveDocAccess`; EDITOR writes, anyone else is read-only, deleted versions are refused) and stores state in `note_document_markup` with raw SQL. The persistence adapter's markup branches skip workspaces, Redis and entity references, never write back a room whose load failed, and skip empty saves. `src/core/markupSync.ts` keeps a per-device registry of versions with unsynced changes, flushes them after sign-in and on `online`, and wipes markup on sign-out (warning first if anything is unsynced).
